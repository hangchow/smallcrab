...
public class ApacheScanner implements LineScanner {
	...
	public String[] scan(String line) {
		ApacheLogSpliter spliter = new ApacheLogSpliter();
		spliter.split(line);// split and store segments
		boolean allMatched = true;
		if (lineMatchers != null && lineMatchers.size() != 0)
			for (LineMatcher<ApacheLogSpliter> lineMatcher : lineMatchers) {
		...
		int index = 0;
		for (LineViewer<ApacheLogSpliter> lineViewer : lineViewers) {
